...
**Design Rationale**:
- **Users** represent individual accountability (multiple allowed for collaboration)
- **Teams** represent organizational ownership (single source of authority)
- **Mutual exclusion** prevents ambiguity in ownership hierarchy
...
- **JSON Schema**: `openmetadata-spec/.../type/ownerConfig.json`

### Code
- **Implementation**: `ingestion/src/metadata/utils/owner_utils.py`
- **Unit Tests**: `ingestion/tests/unit/test_owner_utils.py`
- **Integration Tests**: `ingestion/tests/integration/owner_config_tests/test-*.yaml`

### Common Commands
```bash
# Format Python code
cd /workspace/ingestion && make py_format

# Build JSON Schema
cd /workspace/openmetadata-spec && mvn clean install
